...
ARM_SETTLE_TOLERANCE = float(os.getenv("AGENT_ARM_SETTLE_TOLERANCE", "0.03"))
ARM_TASK_TIMEOUT = float(os.getenv("AGENT_ARM_TASK_TIMEOUT_SIM_S", "30"))
HOME_ARM = [0.0, 1.57, -2.635, 1.78, 0.0]
READY_ARM = [0.0, 0.0, -0.77, -1.21, 0.0]
PUSH_ARM = [0.0, -0.34, -1.05, -0.85, 0.0]
with (WORKSPACE / "config" / "locations.json").open(
    encoding="utf-8"
) as stream:
    _LOCATION_CONFIG = json.load(stream)
NAMED_POSES = {
    name: (values["x"], values["y"], values["yaw"])
    for name, values in _LOCATION_CONFIG.items()
}
NAV_ROUTES = {
    ("home", "indicator_station"): [
        NAMED_POSES["indicator_station"][:2],
    ],
    ("indicator_station", "home"): [
        NAMED_POSES["home"][:2],
    ],
    ("indicator_station", "main_switch"): [
        NAMED_POSES["main_switch"][:2],
    ],
    ("main_switch", "indicator_station"): [
        NAMED_POSES["indicator_station"][:2],
    ],
    ("home", "main_switch"): [
        NAMED_POSES["indicator_station"][:2],
...
class AgentTaskController:
    def __init__(self):
        ...
        self.node = rclpy.create_node("agent_webots_bridge")
        ...
        self.indicator_led = self.robot.getFromDef(
            "INDICATOR_LED_APPEARANCE"
        )
        ...
        self.arm_status_pub = self.node.create_publisher(String, "/agent_task/arm_status", 10)
        self.nav_status_pub = self.node.create_publisher(String, "/agent_task/nav_status", 10)
        self.switch_pub = self.node.create_publisher(String, "/agent_task/switch_state", 10)
        self.state_pub = self.node.create_publisher(String, "/agent_task/robot_state", 10)
        ...
        self.node.create_subscription(
            String, "/agent_task/arm_command", self.on_arm_command, 10
        )
        self.node.create_subscription(
            String, "/agent_task/nav_command", self.on_nav_command, 10
        )
        self.node.create_subscription(
            String, "/agent_task/stop", self.on_stop, 10
        )
        ...
        self.arm_task = None
        self.nav_task = None
        self.arm_status_request_id = None
        self.nav_status_request_id = None
        self.location = "home"
        self.arm_pose = "home"
        self.stopped = False
        self.switch_state = os.getenv("AGENT_TASK_SWITCH_INITIAL", "on").lower()
        self.nav_drive_command = DriveCommand(0.0, 0.0, 0.0)
        self.diagnostic_path = os.getenv("AGENT_TASK_POSE_DIAGNOSTICS")
        self.diagnostic_file = None
        self.autostart_target = os.getenv("AGENT_TASK_AUTOSTART_NAV")
        self.autostart_pending = self.autostart_target in NAMED_POSES
        self.quit_after_navigation = os.getenv("AGENT_TASK_QUIT_AFTER_NAV") == "1"
        diagnostic_twist = os.getenv("AGENT_TASK_DIAGNOSTIC_TWIST")
        ...
        if self.diagnostic_twist is not None and len(self.diagnostic_twist) != 3:
            raise ValueError("AGENT_TASK_DIAGNOSTIC_TWIST requires vx,vy,omega")
        self.diagnostic_quit_time = float(
            os.getenv("AGENT_TASK_DIAGNOSTIC_QUIT_TIME", "0")
        )
    ...
    def _configure_devices(self):
        ...
        rotation_override = os.getenv("AGENT_TASK_CAMERA_ROTATION")
        if rotation_override:
            rotation = [float(value) for value in rotation_override.split(",")]
            if len(rotation) != 4:
                raise ValueError("AGENT_TASK_CAMERA_ROTATION requires four values")
    ...
    def _report_devices(self):
        report = {
            ...
            "indicator_led": self.indicator_led is not None,
    ...
    def on_arm_command(self, message):
        try:
            ...
            return
        self.arm_status_request_id = command.get("request_id", "arm-action")
        if command.get("action") != "move_arm_to_named_pose":
            self._publish_arm_status("rejected", "unsupported arm action")
            return
        target_pose = command.get("pose")
        if target_pose not in {"home", "ready", "press", "retract"}:
            self._publish_arm_status("rejected", "unknown named arm pose")
            return
        ...
        try:
            ...
            self._publish_arm_status("rejected", str(error))
            return
        if target_pose in {"ready", "press"}:
            distance, heading_error = self._switch_pose_error()
        else:
            distance, heading_error = 0.0, 0.0
        if distance > 0.72 or heading_error > 0.38:
            ...
            return
        self.arm_task = {
            "request_id": self.arm_status_request_id,
        ...
        self._set_arm_pose(pose_values)
        self._publish_arm_status(
            "running", f"moving arm to named pose {target_pose}"
        )
    ...
    def on_stop(self, message):
        ...
        self.arm_pose = "home"
        self.stopped = True
        detail = message.data or "stop requested"
        self._publish_nav_status("cancelled", detail)
        self._publish_arm_status("cancelled", detail)
    ...
    def on_nav_command(self, message):
        try:
            ...
            return
        self.nav_status_request_id = command.get("request_id", "navigation")
        ...
        if route is None:
            ...
            return
        self.nav_task = {
            "request_id": self.nav_status_request_id,
            "target": target,
            "points": route,
            "segment": 0,
    ...
    def _switch_pose_error(self):
        ...
        switch_x, switch_y, switch_yaw = NAMED_POSES["main_switch"]
        heading_error = math.atan2(
            math.sin(switch_yaw - yaw),
            math.cos(switch_yaw - yaw),
        )
        return (
            math.hypot(switch_x - x, switch_y - y),
            abs(heading_error),
        )
    ...
    def _set_switch_visuals(self):
        ...
        self.indicator_led.getField("baseColor").setSFColor(color)
        self.indicator_led.getField("emissiveColor").setSFColor(color)
    ...
    def _publish_arm_status(self, status, detail):
        ...
        elif self.arm_status_request_id:
            payload["request_id"] = self.arm_status_request_id
    ...
    def _publish_nav_status(self, status, detail):
        ...
        elif self.nav_status_request_id:
            payload["request_id"] = self.nav_status_request_id
    ...
    def _update_arm_task(self):
        ...
        target_pose = self.arm_task["target_pose"]
        pose_values = {
            "home": HOME_ARM,
            "ready": READY_ARM,
            "press": PUSH_ARM,
        ...
        if elapsed >= 0.35 and self._arm_at_pose(pose_values):
            if target_pose == "press":
                self.switch_state = switch_state_after_press(self.switch_state)
                self._set_switch_visuals()
            self.arm_pose = target_pose
            self._publish_arm_status(
                "succeeded", f"reached named arm pose {target_pose}"
            )
            self.arm_task = None
        elif elapsed >= ARM_TASK_TIMEOUT:
            self._publish_arm_status(
                "failed", f"arm did not settle at named pose {target_pose}"
            )
            self.arm_task = None
    ...
    def _publish_robot_state(self):
        ...
        yaw = ros_yaw_from_webots(self.imu.getRollPitchYaw()[2])
        payload = {
            "captured_at_unix_s": self.node.get_clock().now().nanoseconds / 1e9,
            "location": self.location,
            "arm_pose": self.arm_pose,
    ...
    def _update_nav_task(self):
        ...
        target_yaw = (
            NAMED_POSES[self.nav_task["target"]][2]
