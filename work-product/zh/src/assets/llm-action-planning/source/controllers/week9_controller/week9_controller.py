...
class Week9Controller:
    def __init__(self):
        self.robot = Supervisor()
        self.node = rclpy.create_node("week9_webots_bridge")
        ...
        self.arm_status_pub = self.node.create_publisher(String, "/week9/arm_status", 10)
        self.nav_status_pub = self.node.create_publisher(String, "/week9/nav_status", 10)
        self.switch_pub = self.node.create_publisher(String, "/week9/switch_state", 10)
        ...
        self.node.create_subscription(
            String, "/week9/arm_command", self.on_arm_command, 10
        )
        self.node.create_subscription(
            String, "/week9/nav_command", self.on_nav_command, 10
        )
        ...
        self.nav_task = None
        self.location = "home"
        self.switch_state = os.getenv("WEEK9_SWITCH_INITIAL", "on").lower()
        self.nav_drive_command = DriveCommand(0.0, 0.0, 0.0)
        self.diagnostic_path = os.getenv("WEEK9_POSE_DIAGNOSTICS")
        self.diagnostic_file = None
        self.autostart_target = os.getenv("WEEK9_AUTOSTART_NAV")
        self.autostart_pending = self.autostart_target in {"home", "main_switch"}
        self.quit_after_navigation = os.getenv("WEEK9_QUIT_AFTER_NAV") == "1"
        diagnostic_twist = os.getenv("WEEK9_DIAGNOSTIC_TWIST")
        ...
        if self.diagnostic_twist is not None and len(self.diagnostic_twist) != 3:
            raise ValueError("WEEK9_DIAGNOSTIC_TWIST requires vx,vy,omega")
        self.diagnostic_quit_time = float(
            os.getenv("WEEK9_DIAGNOSTIC_QUIT_TIME", "0")
        )
    ...
    def _configure_devices(self):
        ...
        rotation_override = os.getenv("WEEK9_CAMERA_ROTATION")
        if rotation_override:
            rotation = [float(value) for value in rotation_override.split(",")]
            if len(rotation) != 4:
                raise ValueError("WEEK9_CAMERA_ROTATION requires four values")
