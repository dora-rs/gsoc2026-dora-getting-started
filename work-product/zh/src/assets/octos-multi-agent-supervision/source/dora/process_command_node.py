...
class CommandRuntime:
    ...
    def _navigate(self, request: dict) -> dict:
        ...
        status = self._wait_for_status(
            ...
            timeout_s=float(os.getenv("PROCESS_NAV_TIMEOUT_S", "40")),
        )
        if status is None:
            return failure(
    ...
    def _switch(self, request: dict) -> dict:
        ...
        status = self._wait_for_status(
            ...
            timeout_s=float(os.getenv("PROCESS_SWITCH_TIMEOUT_S", "18")),
        )
        if status is None:
            return failure(
