...
from week11_runtime.control_cycles import (  # noqa: E402
    ControlEngagementTracker,
    StableCompletionGate,
    cap_engagement_counts,
)
from week11_runtime.recording_layout import (  # noqa: E402
    CANVAS_HEIGHT,
    CANVAS_WIDTH,
...
def recorder_should_stop(
    ...
    *,
    completion_gate: StableCompletionGate,
    now: float,
) -> bool:
    return completion_gate.update(
        telemetry,
        engagement_counts,
        now=now,
    )
...
class Week11Recorder(Node):
    def __init__(
        ...
        super().__init__("week11_mission_recorder")
        ...
        self.create_subscription(
            Image,
            "/week11/observer/camera/image_raw",
            self.on_observer,
            10,
        )
        self.create_subscription(
