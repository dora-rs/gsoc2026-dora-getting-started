...
LOCATIONS = json.loads(
    (WORKSPACE / "config" / "process_locations.json").read_text(
        encoding="utf-8"
    )
)


class ProcessRobotController:
    def __init__(self) -> None:
        ...
        if "Observer" in robot_name:
            ...
            role = "operator"
        self.role = role
        self.node = rclpy.create_node(f"process_{self.role}_webots_bridge")
        ...
        self.state_pub = self.node.create_publisher(
            String, f"/process/{self.role}/state", 10
        )
        self.nav_status_pub = self.node.create_publisher(
            String, f"/process/{self.role}/nav_status", 10
        )
        self.arm_status_pub = self.node.create_publisher(
            String, f"/process/{self.role}/arm_status", 10
        )
        self.scan_pub = self.node.create_publisher(
            LaserScan, f"/process/{self.role}/scan", 10
        )
        if self.role == "observer":
            self.camera_pub = self.node.create_publisher(
                Image, "/process/observer/camera/image_raw", 10
            )
        else:
            self.camera_pub = None
        self.switch_pub = self.node.create_publisher(
            String, "/process/operator/switch_event", 10
        )
        self.node.create_subscription(
            String,
            f"/process/{self.role}/nav_command",
            self.on_nav_command,
            10,
        )
        self.node.create_subscription(
            String,
            f"/process/{self.role}/arm_command",
            self.on_arm_command,
            10,
        )
        self.node.create_subscription(
            String, f"/process/{self.role}/stop", self.on_stop, 10
        )
...
def main() -> None:
    ...
    controller = ProcessRobotController()
    try:
        controller.run()
    finally:
        if rclpy.ok():
