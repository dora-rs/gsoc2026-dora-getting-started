...
def plant_config_from_environment(
    ...
    return PlantConfig(
        initial_temperature_c=float(
            environment.get("PROCESS_INITIAL_TEMPERATURE_C", "32.0")
        ),
        initial_pressure_kpa=float(
            environment.get("PROCESS_INITIAL_PRESSURE_KPA", "162.0")
        ),
        heating_rate_c_per_s=float(
            environment.get("PROCESS_HEATING_RATE_C_PER_S", "0.25")
        ),
        pressure_rate_kpa_per_s=float(
            environment.get("PROCESS_PRESSURE_RATE_KPA_PER_S", "0.32")
        ),
        cooling_effect_c_per_s=float(
            environment.get("PROCESS_COOLING_EFFECT_C_PER_S", "-1.35")
        ),
        relief_effect_kpa_per_s=float(
            environment.get("PROCESS_RELIEF_EFFECT_KPA_PER_S", "-3.50")
        ),
    )
