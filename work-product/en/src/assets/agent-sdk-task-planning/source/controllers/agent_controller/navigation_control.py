...
def compute_drive_command(
    *,
    ...
    corridor_yaw: float = 0.0,
    corridor_yaw_tolerance: float = 0.1,
    ...
    if distance <= position_tolerance:
        ...
        return DriveCommand(
            0.0,
            0.0,
            _clamp(angular_gain * yaw_error, max_angular_speed),
        )

    yaw_error = _angle_error(corridor_yaw, current_yaw)
    correction = _clamp(
        angular_gain * yaw_error, max_angular_speed
    )
    if abs(yaw_error) > corridor_yaw_tolerance:
        return DriveCommand(0.0, 0.0, correction)

    speed = min(max_linear_speed, linear_gain * abs(dx))
    body_vx = math.copysign(speed, dx)
    return DriveCommand(
        body_vx,
        0.0,
        correction,
