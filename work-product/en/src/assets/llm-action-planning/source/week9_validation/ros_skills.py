...
NAVIGATION_TIMEOUT_S = float(
    os.getenv("WEEK9_NAVIGATION_TIMEOUT_S", "180")
)
ARM_ACTION_TIMEOUT_S = float(
    os.getenv("WEEK9_ARM_ACTION_TIMEOUT_S", "75")
)
NAMED_ROUTES = {
    ("home", "main_switch"): [
        (-2.35, -1.80),
...
class RosSkillRuntime(Node):
    def __init__(self, output_dir: Path):
        super().__init__("week9_dora_skill_runtime")
        ...
        self.arm_pub = self.create_publisher(String, "/week9/arm_command", 10)
        self.nav_pub = self.create_publisher(String, "/week9/nav_command", 10)
        self.create_subscription(Odometry, "/odom", self._on_odom, 10)
        self.create_subscription(Image, "/camera/image_raw", self._on_image, 10)
        self.create_subscription(
            String, "/week9/arm_status", self._on_arm_status, 10
        )
        self.create_subscription(
            String, "/week9/nav_status", self._on_nav_status, 10
        )
        self.create_subscription(
            String, "/week9/switch_state", self._on_switch_state, 10
        )
